...
equivalent), route guard component.

1. Write API-03, API-04, API-05 (`tests.md` Section 4) against the *middleware*, not a specific
   endpoint yet — the header-validation logic in `api-spec.md` Section 2 is shared by every
   Requester-scoped route, so build and test it once, centrally.
2. Implement `GET /api/requesters/active` (`api-spec.md` Section 6.1) and the shared middleware.
   Make API-03/04/05 pass.
...
**Done when:** AC-02 passes end-to-end by hand; API-03/04/05 and UI-10 pass; the shell shows the
...
3. Implement `POST /api/tickets` per `api-spec.md` Section 6.4, including every listed error case
   ...
   3 retry-exhausted attempts). Make API-01/02 pass.
...
2. Implement `GET /api/tickets` per `api-spec.md` Section 6.5 — search, filters, sort, pagination,
   the exact default sort and clamping behavior from BR-12. Make API-06–09 pass.
...
5. Write STYLE-02 (badge non-color accessibility) and RESP-01 (table→card at mobile); make them
   pass.
...
3. Implement upload (`api-spec.md` 6.7, with the safe-filename policy from Section 5 and the
   ...
   double-removal). Make API-13–20 pass.
...
1. Re-run API-10, API-20, and add any missing cross-Requester negative test for an endpoint that
   didn't get one during its own phase.
