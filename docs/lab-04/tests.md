...
| **API-L4-14b** | API | BR-11, D13 | Closed or resolved ticket with mismatched or non-existent actionId | 404 Not Found (`NOT_FOUND`) takes precedence over 400 terminal lock | `server/tests/lab-04/actions-taken.api.test.ts` | Implemented |
...
`Implemented` means a test body exists. `API-L4-30b` is the eligible-assignee lookup test; `API-L4-30` remains the Admin dashboard test. The lookup test title was aligned with this matrix. In F2 implementation verification, `UI-L4-13` and `UI-L4-13b` verified both local initialization and fixed-clock `max = local now + 5m` boundary (8/8 pass in `ActionsTaken.test.tsx`). `API-L4-14b` verified 404 precedence on closed/resolved tickets with mismatched or non-existent actions, and `API-L4-22b` through `API-L4-22e` verified version guards, empty complete results, and follow-up notes (33/33 pass in `actions-taken.api.test.ts`). Full client suite passed at 92/92 (100%).
