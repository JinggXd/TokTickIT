...
- **FR-01 (Action Creation & Attributes):** Permitted IT Staff and Administrators can add an Action Taken under any accessible Ticket, specifying `actionDateTime` (defaults to now, <= now + 5m), `actionDescription` (1–1000 chars), `status` (`PENDING` or `COMPLETED`), optional `assigneeId`, `result` (mandatory if completed), `followUpRequired`, `followUpNote` (required if follow-up checked), `attachmentNotes` (max 500 chars), and optional `clientRequestId` (UUIDv4).
...
- **FR-03 (Action Assignment & Inactive Rejection):** Actions can be assigned to an active IT Staff or Administrator. If `assigneeId` targets a non-existent, inactive, or non-staff user, the backend rejects with `422 Unprocessable Entity` (`INVALID_ASSIGNEE`).
...
- **BR-08 (Action Event Time Validation):** `actionDateTime` cannot exceed current server time plus a 5-minute clock-skew tolerance (`actionDateTime <= now() + 5m`). Future dates beyond tolerance are rejected with `400 Bad Request`.
